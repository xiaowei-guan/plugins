...
VideoPlayer::VideoPlayer(FlutterDesktopPluginRegistrarRef registrar_ref,
  ...
  LOG_DEBUG("[PlusPlayer]call create to create player");
...
void VideoPlayer::PlusPlayerEventListener::OnEos(UserData userdata) {
  LOG_DEBUG("OnEos");
  if (handler_->event_sink_) {
    flutter::EncodableMap encodables = {{flutter::EncodableValue("event"),
                                         flutter::EncodableValue("completed")}};
    flutter::EncodableValue eventValue(encodables);
    LOG_INFO("[VideoPlayer.OnEos] send completed event");
    handler_->event_sink_->Success(eventValue);

    LOG_DEBUG("[VideoPlayer.OnEos] change player state to pause");
    handler_->pause();
  }
}
...
void VideoPlayer::PlusPlayerEventListener::OnSeekDone(UserData userdata) {
  ...
  if (handler_ != nullptr && handler_->on_seek_completed_) {
    handler_->on_seek_completed_();
    handler_->on_seek_completed_ = nullptr;
  }
}
